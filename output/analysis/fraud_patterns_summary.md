...
We found **$355.8 billion** in suspicious spending at the provider level, and another **$116.1 billion** in system-wide rate problems. These are upper-bound estimates — the most that could be at risk — not confirmed losses. A single provider can show up in more than one pattern, so the pattern totals add up to more than the overall total.
...
The biggest outliers include GUARDIANTRAC. LLC ($1.32 billion in suspicious spending), NEW PARTNERS, INC ($912 million in suspicious spending), and CONSUMER DIRECT CARE NETWORK VIRGINIA ($675 million in suspicious spending).
...
### 2. Middleman Billing Organizations — $36.5B, 1,915 providers
...
The biggest outlier is LOS ANGELES COUNTY DEPARTMENT OF MENTAL HEALTH in CA ($5.47 billion in suspicious spending), which triggered 21 different detection methods. Other examples include DEPARTMENT OF INTELLECTUAL AND DEVELOPMENTAL DISABILITIES, STATE OF TN ($2.31 billion) and DEPARTMENT OF INTELLECTUAL AND DEVELOPMENTAL DISABILITIES, STATE OF TN ($1.36 billion).
...
### 3. Government Agencies as Outliers — $53.5B, 20,583 providers
...
11 of the top 20 flagged providers are government agencies — county departments, state agencies, or public institutions. These are not criminals stealing money. They are large public organizations whose billing looks very different from private-sector providers.

The biggest: LOS ANGELES COUNTY DEPARTMENT OF MENTAL HEALTH ($5.47 billion in suspicious spending, flagged by 21 methods — more than any other provider). Tennessee's Dept of Intellectual and Developmental Disabilities totals $3.67 billion across two separate billing IDs.
...
### 4. Providers That Cannot Exist — $929.5M (individual), $116.1B (system-wide)

407 providers billed Medicaid after their billing ID was shut down, or they have no name, specialty, or organization type on file. At the system level, 2,701 entries use fake or non-standard identifiers.
...
### 5. Billing Every Single Day — $9.6B, 20 providers

20 providers billed Medicaid every calendar day — or nearly every day — across multiple years. The list includes LOS ANGELES COUNTY DEPARTMENT OF MENTAL HEALTH, GUARDIANTRAC. LLC, NEW PARTNERS, INC, COUNTY OF SANTA CLARA, CONCEPTS OF INDEPENDENCE, INC., all of which also appear in other patterns.
...
### 6. Sudden Starts and Stops — $91.8B, 2,433 providers

![Top 5 Flagged Providers: Monthly Billing Anomaly Patterns](../charts/temporal_anomalies_top5.png)

This is the biggest pattern by dollar amount. It catches providers that appeared out of nowhere with high billing, disappeared abruptly, or had sudden spikes or drops in their monthly totals. Examples: GUARDIANTRAC. LLC in MI ($1.32 billion), CITY OF CHICAGO in IL ($1.16 billion), MAINS'L FLORIDA, INC. in MN ($973 million).
...
### 7. Billing Networks and Circular Billing — $11.3B, 846 providers
...
The most extreme case: LOS ANGELES COUNTY DEPARTMENT OF MENTAL HEALTH (registered in CA, $5.47 billion in suspicious spending).

**What it means:** These network patterns can indicate kickback schemes, shell companies, or billing setups designed to inflate payments.

### 8. State-Level Spending Differences — $77.5B (system-wide only)

![Flagged Spending by State](../charts/state_heatmap.png)

20 combinations of state and procedure code where spending per patient is more than double the national average. The biggest: New York's personal care code T1019, where spending is $3,159 per patient versus the national median of $1,526 (2.1 times higher, $74.6 billion total). New Jersey's comprehensive community support code: $6.9 billion. New York's personal care per-day code: $3.5 billion.
...
### 9. Upcoding and Impossible Volumes — $3.4B, 36 providers
...
Examples: COUNTY OF ORANGE, CA ($667 million), SCOTTISH RITE CHILDREN'S MEDICAL CENTER, GA ($388 million), NORTH SHORE-LIJ MEDICAL PC, NY ($298 million), THE CLEVELAND CLINIC FOUNDATION, OH ($247 million).
...
### 10. Shared Beneficiary Counts — $2.4B, 19 providers
...
Top example: COMMUNITY ASSISTANCE RESOURCES & EXTENDED SERVICES INC in NY ($803 million, flagged by 13 methods). Massachusetts state agencies also appear here.
...
1. **Government agencies dominate the risk list.** 11 of the top 20 flagged providers are public agencies. This is not a fraud wave — it is a systemic problem with how states set rates and structure billing for their own agencies.
...
3. **Multiple independent signals are the best indicator.** 97% of the top 500 providers triggered 3 or more different detection methods. A single flag is a screening lead. Three or more flags together are worth investigating.
